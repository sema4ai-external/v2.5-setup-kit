// TEMPLATE A — MCP GALLERY ONLY, LEAST ACCESS (delegated).
// (For the combined gallery + blockparty app, see sema4-m365-graph-app-combined.bicep.)
//
// Entra app registration for the Sema4 M365 / Microsoft Graph MCP servers.
//
// Models the REAL production OAuth app "Sema4.ai" (client_id
// 31731790-4d45-4ab6-8ab7-30813caab8c3) — the app the microsoft-* MCP servers
// authenticate against (referenced as microsoftOauthClientId in sema4ai-gateway,
// multi-tenant, /common endpoint).
//
// Scope set is TRIMMED to exactly the delegated (act-as-user) permissions the six
// microsoft-* MCP servers in mcp-gallery declare — a subset of the live app's full
// 44-scope grant. [user] = user-consentable; [ADMIN] = tenant admin consent.
// After deploy, grant the admin scopes with:
...
// Delegated Graph permissions the microsoft-* MCP servers request.
...
// For-expressions are only valid as variable/property values (not concat args).
var graphResourceAccess = [
  for scope in items(delegatedScopes): {
    id: scope.value
    type: 'Scope'
  }
]
